...
const AddContact = async function({user, name, email, phone, job, group, active, starred }) {
    try{
        const temp = await new Contact({ user, name, email, phone, job, group, active }).save();
        return temp
    }catch(Error) {
        throw Error
    }
...
const EditContact = async function({id, user, name, email, phone, job, group, active, starred }) {
    try {
        const temp = await Contact.findByIdAndUpdate(id, {user, name, email , phone, job, group, active})
        return temp
    }catch(Error) {
        throw Error
    }
...
const GetGroup = async function({id}) {
    try {
        const temp = await Group.findById(id)
        return temp
    }catch(Error) {
        throw Error
    }
...
module.exports = {  Connect, Login, Register, 
                    ...
                    GetGroups, GetGroup, AddGroup, EditGroup, DeleteGroup, ActivateGroup,
                    GetCurrentUser }
